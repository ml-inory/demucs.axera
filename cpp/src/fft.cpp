...
Fft::vec_complex Fft::ifft(const Fft::vec_complex &signal, size_t vec_begin, size_t vec_size, bool in_place) {
	Fft::vec_complex::const_iterator first = signal.begin() + vec_begin;
	Fft::vec_complex::const_iterator last = signal.begin() + vec_begin + vec_size;

	// 1/N * conj(FFT(conj(x)))
	Fft::vec_complex reverse_freq_spectrum(first, last);
	// conj(x)
	std::transform(reverse_freq_spectrum.begin(), reverse_freq_spectrum.end(), reverse_freq_spectrum.begin(),
					[](const std::complex<float> &num) { return std::conj(num); });
	// fft
	auto comp_result = fft(reverse_freq_spectrum, 0, reverse_freq_spectrum.size(), in_place);
	// // conj(fft)
	const float len = comp_result.size();
	std::transform(comp_result.begin(), comp_result.end(), comp_result.begin(),
					[len](const std::complex<float> &num) { return std::conj(num) / len; });
...
Fft::mat_complex Fft::stft_ifft(Fft::vec_complex &vec, size_t window_size, size_t window_step, bool in_place) {
	Fft::mat_complex spectrogram = Fft::mat_complex();
	for (size_t begin = 0; begin < vec.size() - window_size; begin+=window_step)
	{
		spectrogram.push_back(Fft::ifft(vec, begin, window_size, in_place));
	}
	return spectrogram;
}
