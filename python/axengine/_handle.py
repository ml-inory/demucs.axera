...
class InferenceSession:
    ...
    @classmethod
    def load_from_mem(cls, data: np.ndarray) -> "InferenceSession":
        """
        Load model graph to InferenceSession.

        Args:
            data (np.ndarray): model data in mem
        """
        _handle = _C.Runner()
        sess = cls(_handle)
        success = sess._handle.load_model_from_mem(data.tobytes())
        if not success:
            raise BufferError("Err... Something wrong while loading the model.")
        success = sess._handle.set_io_buffer()
        if not success:
    ...
    def set_runtime_context(self, group_id: int) -> None:
        if self.num_io_groups == 1:
            logger.warning("No need to reset the Group ID as there is only one I/O group..")
            return

        if group_id < 0 or group_id >= self.num_io_groups:
            raise IndexError(f"Group ID must be between 0 and {self.num_io_groups - 1}.")

        self._handle.set_io_shape_group(group_id)
        success = self._handle.set_io_buffer()
        if not success:
            raise BufferError("Failed to set the model runtime context.")
