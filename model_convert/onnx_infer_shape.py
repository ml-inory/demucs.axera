...
class SymbolicShapeInference:
    def __init__(self, int_max, auto_merge, guess_output_rank, verbose, prefix=""):
        self.dispatcher_ = {
            ...
            "Gelu": self._infer_Gelu,  # noqa: F601
    ...
    def _onnx_infer_subgraph(
        ...
        subgraph_implicit_input = set(
            [name for name in self.known_vi_.keys() if name not in subgraph_inputs]
        )
        tmp_graph = helper.make_graph(
            list(subgraph.node),
            "tmp",
        ...
        subgraph_new_symbolic_dims = set(
            [
                ...
                if s
                for d in s
                if isinstance(d, str) and d not in self.symbolic_dims_
            ]
        )
        new_dims = {}
    ...
    def _infer_aten_bce(self, node):
        ...
        if reduction == 0:
            ...
            vi.CopyFrom(
                helper.make_tensor_value_info(
                    vi.name, elem_type, self._get_shape(node, 0)
                )
            )
    ...
    def _infer_Slice(self, node):
        ...
        if (
            ...
            if isinstance(input_sympy_data, list) or (
                isinstance(input_sympy_data, np.array)
